...
class Agent(object):
    ...
    def set_full(self, px, py, vx, vy, radius, gx, gy, v_pref, theta):
        self.px = px
        self.py = py
        self.vx = vx
        self.vy = vy
    ...
    def get_next_full_state(self, action):
        ...
        if self.kinematics == 'holonomic':
            ...
            next_vx = action.v * np.cos(next_theta)
            next_vy = action.v * np.sin(next_theta)
        return FullState(next_px, next_py, next_vx, next_vy, self.radius, self.gx, self.gy, self.v_pref, self.theta)
